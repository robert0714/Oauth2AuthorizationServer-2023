...
//				.param(OAuth2ParameterNames.USERNAME, "91F147")
//				.param(OAuth2ParameterNames.PASSWORD, "Ie=123456789")
		...
		System.err.println("\nRefresh token from 'custom password' grant -> " + refreshToken);
	...
	private static OAuth2RefreshToken getRefreshToken(MvcResult mvcResult) {
		MockHttpServletResponse servletResponse = mvcResult.getResponse();
		MockClientHttpResponse httpResponse = new MockClientHttpResponse(
				servletResponse.getContentAsByteArray(), HttpStatus.valueOf(servletResponse.getStatus()));
		try {
			return accessTokenHttpResponseConverter.read(
					OAuth2AccessTokenResponse.class, httpResponse).getRefreshToken();
		} catch (Exception ex) {
			System.err.println("FOUT " + ex.getMessage());
			throw new RuntimeException(ex);
		}
